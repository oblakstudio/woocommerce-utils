...
abstract class Extended_Data extends \WC_Data {
    ...
    public function __call( $name, $arguments ) {
        if ( ! preg_match( '/^(get|set)_/', $name ) ) {
            throw new BadMethodCallException( sprintf( 'Method %s::%s does not exist', static::class, esc_html( $name ) ) );
        }

        $property = preg_replace( '/^(get|set)_/', '', $name );

        if ( ! in_array( $property, $this->get_data_keys(), true ) ) {
            throw new BadMethodCallException( sprintf( 'Property %s does not exist', esc_html( $property ) ) );
        }
    ...
    protected function set_prop( $prop, $value ) {
        ...
        match ( $prop_type ) {
            ...
            'bool' => '' !== $value && parent::set_prop( $prop, wc_string_to_bool( $value ) ),
    ...
    protected function get_prop( $prop, $context = 'view' ) {
        ...
        return match ( true ) {
            str_ends_with( $prop, '_gmt' ) && 'date' === $type => gmdate( 'Y-m-d H:i:s', $value->getTimestamp() ),
            'date' === $type => gmdate( 'Y-m-d H:i:s', $value->getOffsetTimestamp() ),
            'bool' === $type => wc_bool_to_string( $value ),
            default => $value,
        };
    }
    ...
    protected function is_internal_meta_key( $key ) {
        ...
        if ( ! $parent_check && in_array( $key, $this->get_data_keys(), true ) ) {
            ...
            wc_doing_it_wrong( __FUNCTION__, sprintf( __( 'Generic add/update/get meta methods should not be used for internal meta data, including "%s". Use getters and setters.', 'woocommerce' ), $key ), 'WooCommerce Utils - 3.2.0' );
            return true;
        }
