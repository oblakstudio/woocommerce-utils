<?php
/**
 * Filter_Manager class file.
 *
 * @package WooCommerce Sync Supplier
 */

namespace Oblak\WSS\Utils;

/**
 * Filter Manager is a simple class for managing wp filters and hooks.
 *
    ...
    public static function find_class_with_filter( string $hook_name, string $class_name, string $method_name, int $priority = 10 ): ?string {
        ...
        foreach ( $wp_filter[ $hook_name ]->callbacks[ $priority ] as $id => $filter_array ) {
            if (
                ...
                ! is_object( $filter_array['function'][0] ?? null ) || // If not an object, it's not a class/method.
                ! get_class( $filter_array['function'][0] ?? null ) || // If not a class, it's not a class/method.
                get_class( $filter_array['function'][0] ) !== $class_name || // If not the right class, it's not a class/method.
